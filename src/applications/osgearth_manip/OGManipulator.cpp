...
osg::Node* OGManipulator::getNode()
{
    return _node.get();
}

const osg::Node* OGManipulator::getNode() const
{
    return _node.get();
}

void OGManipulator::flushMouseEventStack()
{
    _ga_t1 = NULL;
    _ga_t0 = NULL;
}
...
void OGManipulator::addMouseEvent(const GUIEventAdapter& ea)
{
    _ga_t1 = _ga_t0;
    _ga_t0 = &ea;
}

void OGManipulator::setNode(osg::Node* node)
{
    _node = node;
...
void OGManipulator::jumpTo(const osg::Vec3d &center, double distance, double heading, double pitch, double time)
{
...
void OGManipulator::setDistance(const double &distance, bool smooth)
{
...
void OGManipulator::setPitchSmoothing(const double &smoothing)
{
    _pitchSmoothing = osg::clampBetween(smoothing, 0.0, 1.0);
}

void OGManipulator::setHeadingSmoothing(const double &smoothing)
{
    _headingSmoothing = osg::clampBetween(smoothing, 0.0, 1.0);
}

void OGManipulator::setCenterSmoothing(const double &smoothing)
{
    _centerSmoothing = osg::clampBetween(smoothing, 0.0, 1.0);
}

void OGManipulator::setDoubleClickZoomRatio(const double &ratio)
{
    _doubleClickZoomRatio = ratio;
}


void OGManipulator::setCenter(const osg::Vec3d &center, bool smooth)
{
    if (!smooth)
    {
...
void OGManipulator::setHeading(const double &heading, bool smooth)
{
    if (!smooth)
    {
...
void OGManipulator::setPitch(const double &pitch, bool smooth)
{
    _goalPitch = osg::clampBetween(pitch, -osg::PI_2, 0.0);
    if (!smooth)
    {
...
bool OGManipulator::getIntersectionAtLatLon(const double &latRadians, const double &lonRadians, osg::Vec3d &hit)
{
    if ( _csn.valid() && _csn->getEllipsoidModel())
    {
        osg::Vec3d loc;
...
bool OGManipulator::getIntersectionAtLocation(const osg::Vec3d &loc, osg::Vec3d& hit)
{
    if (_node.valid())
    {
...
void OGManipulator::setByMatrix(const osg::Matrixd& matrix)
{
...
osg::Matrixd OGManipulator::getMatrix() const
{
    return osg::Matrixd::inverse(getInverseMatrix());
}

osg::Matrixd OGManipulator::getInverseMatrix() const
{
    return 
        getRotation(_center) *
...
osg::Matrixd OGManipulator::getRotation(const osg::Vec3d &center) const
{
...
void OGManipulator::home(const GUIEventAdapter& ,GUIActionAdapter& us)
{
...
void OGManipulator::computeHomePosition()
{
    if(getNode())
    {
...
double OGManipulator::getRadius()
{
    if (_csn.valid() && _csn->getEllipsoidModel())
    {
...
void OGManipulator::init(const GUIEventAdapter& ,GUIActionAdapter& )
{
    flushMouseEventStack();
}

void OGManipulator::goNorthUp()
{
    setHeading(0, _smoothingEnabled);
}

void OGManipulator::lookAtXY(const double &x, const double &y)
{
    if (!_node) return;
...
bool OGManipulator::handle(const GUIEventAdapter& ea,GUIActionAdapter& us)
{
...
bool OGManipulator::intersect(const osg::Vec3d& start, const osg::Vec3d& end, osg::Vec3d& intersection) const
{
...
void OGManipulator::computePosition(const osg::Vec3d& eye,const osg::Vec3d& center,const osg::Vec3d& up)
{
    if (!_node) return;
...
osgEarth::Viewpoint OGManipulator::getCurrentViewpoint()
{
    double x = _center.x();
    double y = _center.y();

    //If the model is geocentric, the X and Y of the viewpoint is the lon and lat in degrees
    if (_csn.valid() && _csn->getEllipsoidModel())
    {
        double height;
        _csn->getEllipsoidModel()->convertXYZToLatLongHeight(_center.x(), _center.y(), _center.z(), y, x, height);
        _center.x() = osg::RadiansToDegrees( _center.x() );
        _center.y() = osg::RadiansToDegrees( _center.y() );

    }
    return osgEarth::Viewpoint( _center, _heading, _pitch, _distance);
}

void OGManipulator::setViewpoint(const osgEarth::Viewpoint& viewpoint, double time /*= 0.0*/)
{
    osg::Vec3d newCenter;
    //If the model is geocentric, the X and Y of the viewpoint is the lon and lat in radians
    if (_csn.valid() && _csn->getEllipsoidModel())
    {
        osg::Vec3d focalPoint = viewpoint.getFocalPoint();
        _csn->getEllipsoidModel()->convertLatLongHeightToXYZ( osg::DegreesToRadians(focalPoint.y()), osg::DegreesToRadians(focalPoint.x()), 0, newCenter.x(), newCenter.y(), newCenter.z());
    }
    else
    {
        newCenter.x() = viewpoint.getFocalPoint().x();
        newCenter.y() = viewpoint.getFocalPoint().y();
        newCenter.z() = 0;
    }
    ...
    jumpTo(newCenter, viewpoint.getRange(), viewpoint.getHeading(), viewpoint.getPitch(), time);
}

bool OGManipulator::update(double dt)
{
    bool finished = true;
...
double OGManipulator::getHeightAboveTerrain()
{
    if (_csn.valid())
    {
